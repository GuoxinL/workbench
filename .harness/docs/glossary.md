...
| **脏标记** | dirty / 待推送 | 待同步标记 | 「本地存在尚未推送到 GitHub 的领先变更」。**当前实现中不再是持久化的 localStorage 标志位**：同步引擎每轮 `doSync` 经 `planDiff(local, manifest)` 算出本地相对远端有哪些 id 需要 `push`，无领先变更则整轮不写远端（避免空 commit）；本地优先写已先落 IndexedDB，所以变更**不会因未推送而丢失**。任何经 `storageLayer` 的 Save/Delete 都会触发 `schedulePush()` 安排下一轮同步 | `src/services/sync/engine.ts`（`planDiff` / `aPlan.push`）；`src/services/storage/storageLayer.ts`（`schedulePush` 调用） |
...
| **`wb.*` 键族** | — | — | 本项目在 localStorage 占用的 **3 个** key：`wb.data.v1`（主数据快照，即时加载层 + 零配置启动契约）、`wb.cfg.v1`（同步配置，**含明文 token，永不上传**、永不打日志、永不硬编码）、`wb.manifestSha.v1`（远端 `manifest.json` 的乐观锁 sha）。红线「禁止绕过 `stores/data.ts` 直接读写 `wb.*`」针对这 3 个 | `src/stores/data.ts:22-24`（键名常量 `DATA_KEY` / `MANIFEST_SHA_KEY` / `CFG_KEY`） |
...
| **`manifest.json`** | — | — | 远端数据仓库里的**轻量索引**文件，记录各文章/待办的 `updatedAt` 与 `sha`，供同步引擎一次 GET 拿到全量索引再按差异拉正文；`wb.manifestSha.v1` 存其乐观锁 sha | `src/services/github/manifest.ts`、`src/services/github/contents.ts` |
...
| **代码仓库 `workbench`** | **数据仓库 `workbench-data`** | 两个不同的 GitHub 仓库：前者放源码、**必须公开**（GitHub Pages 要求）；后者只放 `kb/<id>.md` + `manifest.json`、`必须私有`（存的是个人待办与文章）。同步指向的永远是后者 | 明确写「代码仓库」/「数据仓库」，禁止只说「仓库」 |
...
| **`data/workbench.json` 单文件数据载体** | 2026-07-31 | `kb/<id>.md` + `manifest.json` | 重构后远端按文章拆分，索引驱动差分同步，降低配额消耗与冲突面 |
...
| **`wb.dirty` / `wb.device.v1` 键** | 2026-07-31 | 引擎内 diff（`planDiff`）；设备标识未沿用 | 脏标记改为每轮 diff 计算；设备维度标识在重构后未保留 |
