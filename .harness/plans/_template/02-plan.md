...
> 持久化结构变更必须同步修改 `.harness/docs/sqls/` 对应文件（DB schema / migration / 协议帧 / 配置 schema 等）。
...
❌ **改动清单过粗** → 逐文件列出（`internal/logic/auth/loginlogic.go` 新增手机号分支、`internal/types/types.go` 新增 `PhoneNumber` 类型），不要"改造 auth 模块"。
