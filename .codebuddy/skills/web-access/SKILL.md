...
```bash
node ".codebuddy/skills/web-access/scripts/check-deps.mjs"
```

**Node.js 22+** 最佳（使用原生 WebSocket）；**Node 20** 亦可，脚本会自动回退到 `ws` 模块（本机已具备）。本 skill 已适配 CodeBuddy Code，路径改用仓库相对的 `.codebuddy/skills/web-access`。

按脚本输出处理：
- `exit 0` → 继续
- `exit 2` → 需询问用户偏好，写入 `.codebuddy/skills/web-access/config.env` 的 `WEB_ACCESS_BROWSER`
...
```bash
node ".codebuddy/skills/web-access/scripts/find-url.mjs" [关键词...] [--only bookmarks|history] [--browser chrome|edge] [--limit N] [--since 1d|7h|YYYY-MM-DD] [--sort recent|visits]
```
