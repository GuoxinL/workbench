...
description: |
  ...
  工作流：启动时定位当前 SOP 任务目录下的 01-clarify.md，阶段一引导发现真实需求，阶段二苏格拉底式挑战假设，最终将需求卡片写入 .harness/plans/<任务目录>/01-clarify.md（不生成 PRD、AI coding prompt 等独立产物）。
...
> ⚠️ **重要**：本项目使用 `.harness/plans/YYYY-MM-DD_<short-title>/` 的 SOP 任务目录体系。
> ⚠️ **重要**：本项目使用 `.harness/plans/YYYY-MM-DD_<short-title>/` 的 SOP 任务目录体系。
...
2. 在 `.harness/plans/` 下查找与当前分支匹配的任务目录：遍历各 `.harness/plans/*/00-overview.md`，匹配 Meta 中 `分支` 字段等于当前分支名的那个目录即为当前任务。
3. 目标文件路径固定为：`.harness/plans/<任务目录>/01-clarify.md`

> ⚠️ 本项目**不维护**全局 `.harness/plan.md`——任务发现完全依赖 `plans/` 目录 + 各自的 `00-overview.md`。
...
1. **保存路径**：启动时定位到的 `.harness/plans/<任务目录>/01-clarify.md`（**唯一目标**，禁止新建其他需求文件）
2. **写入策略**（按启动时用户选择的模式）：
...
4. **保存完成后告知用户**：`已保存至 .harness/plans/<任务目录>/01-clarify.md`
...
Challenge 挑战数：[N 个假设]
输出：需求卡片 ✓
已保存：.harness/plans/<任务目录>/01-clarify.md
...
启动
  ↓
定位当前 SOP 任务目录（基于 git 分支 + 遍历 .harness/plans/*/00-overview.md 的 Meta `分支` 字段）
  ↓
  ├── 未找到任务目录 → 提示用户先按 SOP 建立任务目录，终止流程
  └── 找到 .harness/plans/<任务目录>/01-clarify.md
        ↓
检查 01-clarify.md 现状
  ├── 不存在 / 模板占位 → 新建模式（覆盖写入）
